...
const DefaultLayout = ({ children }: PropsWithChildren) => {
    const isFetchProfile = useRef(false);
    const { animation, navbar, sidebar, toggleSidebar } = useThemeStore(
        useShallow((state) => ({
    ...
    useEffect(() => {
        if (!isFetchProfile.current) {
            getProfile();
            isFetchProfile.current = true;
        }
    }, []);
    ...
    return (
        <App>
            ...
            <div className="relative">
                ...
                <div className={`${(!sidebar && 'hidden') || ''} fixed inset-0 bg-[black]/60 z-50 lg:hidden`} onClick={() => toggleSidebar(false)}></div>
                {/* screen loader */}
                {showLoader && (
