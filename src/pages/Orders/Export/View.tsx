...
const OrderExportView: FC = () => {
    const { dateRef, modeOptions, orderStatusOptions, serviceCategoryOptions, serviceOptions, userOptions, providerOptions, isSubmitting, formState, onChangeFormState, onClearFormState, onSubmit } =
        useViewModel();
        ...
        <Fragment>
            <div className='panel'>
                <div className='grid grid-cols-12 gap-4'>
                    ...
                    <div className='col-span-12 lg:col-span-4'>
                        <label htmlFor='categoryId'>หมวดหมู่บริการ</label>
                        <Select
                            isMulti
                            value={serviceCategoryOptions.filter(option => formState.categoryId.includes(option.value))}
                            onChange={(option: any) => {
                                onChangeFormState(
                                    'categoryId',
                    ...
                    <div className='col-span-12 lg:col-span-4'>
                        <label htmlFor='serviceId'>บริการ</label>
                        <VirtualizedSelect
                            isMulti
                            value={serviceOptions.filter(option => formState.serviceId.includes(option.value))}
                            onChange={(option: any) => {
                                if (!Array.isArray(option)) {
                                    onChangeFormState('serviceId', [...formState.serviceId, option.value]);
                                } else {
                                    onChangeFormState('serviceId', option);
                                }
                            }}
                    ...
                    <div className='col-span-12 lg:col-span-4'>
                        <label htmlFor='status'>สถานะของออร์เดอร์</label>
                        <Select
                            isMulti
                            value={orderStatusOptions.filter(option => formState.status.includes(option.value))}
                            onChange={(option: any) => {
                                onChangeFormState(
                                    'status',
                                    option.map((item: any) => item.value)
                    ...
                    <div className='col-span-12 lg:col-span-4'>
                        <label htmlFor='providerId'>ผู้ให้บริการ</label>
                        <VirtualizedSelect
                            isMulti
                            value={providerOptions.filter(option => formState.providerId.includes(option.value))}
                            onChange={(option: any) => {
                                if (!Array.isArray(option)) {
                                    onChangeFormState('providerId', [...formState.providerId, option.value]);
                                } else {
                                    onChangeFormState('providerId', option);
                                }
                            }}
...
export default OrderExportView;
